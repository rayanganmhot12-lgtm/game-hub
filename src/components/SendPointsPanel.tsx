...
import { sendPointsGift } from "@/lib/pointsRealtime";
...
export default function SendPointsPanel({
  myCode,
  myDisplayName,
  friends,
}: {
  ...
  async function handleSend() {
    ...
    setSending(true);
    try {
      await sendPointsGift(targetCode, Math.floor(amount), myCode, myDisplayName);
      const friend = friends.find((f) => f.friendCode === targetCode);
      showToast(`Sent ${amount} points to ${friend?.friendDisplayName ?? "friend"}.`, "success");
    } catch (err) {
      showToast(err instanceof Error ? err.message : "Couldn't send points.", "error");
    } finally {
      setSending(false);
    }
  }
  ...
  return (
    <div>
      ...
      <p className="mb-3 text-xs text-muted">Developer-only — grant points to a friend, no cost to you.</p>
